...
import {UtilityModelsService} from '../utility-models.service';
...
export class ModeloUtilidadComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.datatableConfig = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.service.getUtilityModels(dataTablesParameters).subscribe(resp => {
          callback(resp);
        });
      },
      columns: [
  ...
  delete(id: number) {
    this.service.deleteUtilityModel(id).subscribe(() => {
