...
export class HistoricalImpiComponent {
  ...
  sheetDataCache: Map<string, { headers: string[], data: any[] }> = new Map(); // Cache para mejorar rendimiento
  ...
  async showSheetPreview(headers: string[], data: any[], sheetName: string) {
    const previewData = data.slice(0, 40);
  ...
  async downloadTemplate() {
    try {
      ...
      link.href = url;
      link.download = 'plantilla-impi.xlsx';
  ...
  async submit() {
    ...
    if (this.form.valid && this.file) {
      ...
      try {
        ...
        // await this.historicalService.uploadData(formData);
  ...
  // --- VALIDACIÓN CONTRA PLANTILLA IMPI ---
...
private extractHeadersFromSheet(wb: XLSX.WorkBook, sheetName: string): string[] {
  ...
  if (!sheet) return [];
  // Usamos el MISMO criterio que tu preview: header en fila 5 (range: 4), columnas desde la 2da (omitimos índice)
...
private compareHeaders(expected: string[], actual: string[]) {
  ...
  // Desorden: solo si no hay faltantes/extra, verificamos orden exacto
...
/**
 * VALIDA QUE COINCIDAN LOS ENCABEZADOS DEL ARCHIVO CARGADO CON LA PLANTILLA IMPI
 */
